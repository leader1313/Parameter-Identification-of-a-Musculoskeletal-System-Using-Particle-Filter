...
[r_f, r_e, angle, angular_velocity, torq] = experiment(extension);
...
%-------------------------- Particle Filter -------------------------------
    ...
    for ip=1:NP
        ...
        w = Gauss4(abs(y(i,1:4)'-(c*x)), b, a);   % calculate a likelihood of a particle
...
save('musculoskeletal6.mat');   %matファイル保存(ファイル名はカッコ内で指定)
end
% ----------------------------- Main loop finish --------------------------
